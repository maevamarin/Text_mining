...
We used the "nrc"  dictionary to start our sentiment analysis on the discourses of the two politicians. It is a dictionnary qualifying tokens by specific sentiments and by labelling them "negative" or "positive".

To do so, we matched the tokens of both corpuses with the nrc dictionnary by applying an inner join. However, to use the inner_join function, we need a table object. We reloaded the data to create objects specific to this stage, boris_2 and macron_2, which are registered as tibble and allowing the use of the inner_join function.
From the token list per document boris.tok, we joined the corresponding qualifier in nrc using an inner_joint: 
...
After creating our objects, we investigated which sentiment are present in Boris Johnson's discourses. To do so, we used a numerical and a graphical method.

The numerical method is simply a matrix of the frequency of tokens identifing with a certain sentiment. We should recall ourselves that one word can have more than one sentiment, which can lead to slight an overestimation of the sentiment.
...
The categories of sentiments are quite similarly distributed: the speeches are positively marked by trust and anticipation.

The proportions in the barplots are the same for speeches 1, 2, and 3. We might assume they are based on a specific inner structure.
...
As the figure \@ref(fig:plot-quanteda) shows, the results of Text 2 as the most positive and all the discourses as majoritarily positive are confirmed. For Macron's speeches, the proportions of sentiments differ: the first discourse is associated with more postive sentiment (224) relatively to proportion computed with the "nrc" dictionnary (193). However, the trend remains the same.

We keep in mind that the difference in the length of the texts explains the varying size of the bars, as previously. Anyway both findings converge to the same points.
...
The results displayed by the figure  \@ref(fig:plot-afinn) are derived from an average score of the sentiments per document. For the Boris Johnson's speeches, the second one has a contradictory score with the previous results: it belongs to the lowest scored text in positive sentiments.

We can thus observe the difference in encodage among dictionnaries used in Text Mining. The "afinn" dictionnary was encoded by a Danish Professor, LSD2015 by two American professors, and "nrc" by a Canadian professors working in an Commision on Ethics. The categorization is subjective and leads to substantial differences. Those results need to be mitigate by a last approach: the use of valence shifters.
...
The sentimentr library offers some function to compute sentiments integrating valence shiters. There are specific words which amplify or reduce the power of a word, or even turn it into the reverse sentiment. We evaluate sentences here, and one important aspect is that it cannot be applied to a Bag Of Word model, since the word order is necessary.

Valence shifters extract more acurately the sentiments of the texts, since they consider the sentiments conveyed by a sentence and not only words without their context. We should take the insights given by valence shifters as the ultimate confirmation test for the previous results. The results are displayed by the plot \@ref(fig:plot-valence-shifter).

The second discourse of the Prime Minister here is one of the longest: it contains about 50 sentences and is only preceded by the fourth public word, on March 18th. It is the one varying the most in sentiment, what might explain the dffference in values observed between LSD2015&nrc with afinn dictionnary. Why? Simply because it was the first speech speaking openly about the covid and the serious measures to take. The sixth one shows one positive peak, what corresponds to the moment when Boris Johnson strengthened the measures and made a speech to reassure the UK citizens. At this time, the measures taken were more rigid: the spread of the virus had intensifyied. He brought information with positive sentiments to unify the population under the new circumstances and motivated his people by showing them the positive impact of respecting stonger measures. 

Macron's speeches are well longer, three times more than Boris Johnson's ones, as we observe in the second plot. His words are generally more neutral but sometimes convey strong negative sentiments, like the 4th sentence. Actually, this sentence is not strongly negative but uses terms which are: "In the vast majority of cases, COVID-19 does not pose a threat, but the virus can have very serious consequences, especially for those of our fellow citizens who are elderly or suffer from chronic illnesses such as diabetes, obesity or cancer". The variation of the sentiments is greater in the last allocution, on April 13th 2020.  Serious + consequences, chronic + illness, diabetes + obesity + cancer aside amplify the seriousness of the sentence. We observe as well a greater variation in sentiments the last allocution of Emmanuel Macon, on April 13th 2020.  It corresponds to the moment when the situation became critical in France. The President emphasizes the serious danger of the Covid-19, the sanitary situation being worse than in UK.
