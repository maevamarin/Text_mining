...
A word embedding is a learned representation for texts where words that have the same meaning have a similar vector representation.
...
Here, we computed the co-occurence matrix. We used the fcm function from quanteda. We used a window lenght 5. 
...
For illustration purpose, we plotted the 50 most used terms as we can observe in the figure \@ref(fig:speech-we).
The closer the words in the embedding space, the more similar they are. Two words are similar if they are often used in the same context.
...
We can observe that there is some coherence within the groups in terms the dates of the speeches. 
...
For illustration purpose, we plotted the 50 most used terms.
